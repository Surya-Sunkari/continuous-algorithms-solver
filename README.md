...
homework_solver/
└── assignment1/
    ├── screenshots/          # Option A: start from screenshots
    │   ├── problem1/
    │   │   ├── page1.png
    │   │   └── page2.png
    │   ├── problem2/
    │   │   └── screenshot.png
    │   └── ...
    ├── problems/             # Option B: start from .tex files directly
    │   ├── p1.tex
    │   ├── p2.tex
    │   └── ...
    └── notes/
...
### 2a. Convert screenshots to LaTeX (if starting from screenshots)

```
/convert assignment1
```

This reads all images in `assignment1/screenshots/problemN/`, transcribes them to exact LaTeX, and writes `assignment1/problems/pN.tex`. Each transcription is verified by a separate subagent for accuracy.
...
The `/convert` skill converts problem screenshots into exact LaTeX:

| Phase | What happens |
|-------|-------------|
| **1. Validation** | Checks that `screenshots/` exists with `problemN/` subfolders |
| **2. Conversion** | Per problem: reads all images → transcribes to LaTeX (opus) → verifies transcription against original (opus) → auto-retries on discrepancies |
| **3. Report** | Lists which problems were converted and any that need manual review |

Each transcription is verified by a separate subagent that compares the LaTeX output symbol-by-symbol against the original screenshot. Up to 3 retries on discrepancies.
...
│       └── convert/
│           └── SKILL.md         # The screenshot-to-LaTeX converter
├── assignment1/
│   ├── screenshots/             # Input for /convert
